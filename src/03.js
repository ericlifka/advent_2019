...
export const day03part1 = () => {
  ...
  input[0].forEach(({direction, distance}) => {
  ...
  x = 0
  y = 0
  input[1].forEach(({direction, distance}) => {
    let [ dx, dy ] = directionVector(direction)
    for (let i = 0; i < distance; i++) {
      x += dx
      y += dy
  ...
  return collisions.reduce((min, p) =>
    (manhattan(p) < min)
      ? manhattan(p)
      : min, Infinity)
}
